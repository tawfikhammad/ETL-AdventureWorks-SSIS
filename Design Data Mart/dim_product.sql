...
IF EXISTS (SELECT *
           ...
           WHERE  NAME = 'fk_fact_sales_dim_product' AND parent_object_id = Object_id('fact_sales'))
...
IF EXISTS (SELECT *
           FROM   sys.objects
           WHERE  NAME = 'dim_product' AND type = 'U')
...
IF EXISTS (SELECT *
  ...
  ALTER TABLE fact_sales
    ADD CONSTRAINT fk_fact_sales_dim_product FOREIGN KEY (product_key) REFERENCES dim_product(product_key);

-- create indexes
IF EXISTS (SELECT *
           FROM   sys.indexes
           WHERE  NAME = 'dim_product_product_id' AND object_id = Object_id('dim_product'))
...
IF EXISTS (SELECT *
           FROM   sys.indexes
           WHERE  NAME = 'dim_prodct_product_category' AND object_id = Object_id('dim_product'))
